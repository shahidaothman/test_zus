<template>
  <v-card>
    <v-card-title>Admin List</v-card-title>
    <v-card-text>
      <div class="overflow-hidden overflow-x-auto min-w-full align-middle sm:rounded-md">
        <div class="flex place-content-end mb-4">
          <div class="px-4 py-2 text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer">
            <router-link :to="{ name: 'admin-create' }" class="text-sm font-medium">Create
              Admin</router-link>
          </div>
        </div>
        ...
        <table class="min-w-full border divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 divide-solid" v-for="item in admins" :key="item.id">
            <template>
              <tr class="bg-white">
...
export default {
  setup() {
    const { admins, getAdmins, destroyAdmin } = useAdmin()
    onMounted(getAdmins)
